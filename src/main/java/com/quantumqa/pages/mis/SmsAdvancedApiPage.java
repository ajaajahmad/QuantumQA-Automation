...
public class SmsAdvancedApiPage extends BasePage {
	...
	public SmsAdvancedApiPage(WebDriver driver) {
		super(driver);
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/sms-advanced-api",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/sms-advanced-api",
					"More_Option_" + i);
