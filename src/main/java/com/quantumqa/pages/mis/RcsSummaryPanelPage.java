...
public class RcsSummaryPanelPage extends BasePage {
	...
	public RcsSummaryPanelPage(WebDriver driver) {
		super(driver);
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/rcs-summary-panel",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/rcs-summary-panel",
					"More_Option_" + i);
