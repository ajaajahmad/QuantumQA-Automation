...
public class TruecallerSummaryApiPage extends BasePage {
	...
	public TruecallerSummaryApiPage(WebDriver driver) {
		super(driver);
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/truecaller-summary-api",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/truecaller-summary-api",
					"More_Option_" + i);
