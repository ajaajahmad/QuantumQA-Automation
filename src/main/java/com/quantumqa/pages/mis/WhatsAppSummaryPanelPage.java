...
public class WhatsAppSummaryPanelPage extends BasePage {
	...
	public WhatsAppSummaryPanelPage(WebDriver driver) {
		super(driver);
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/whatsapp-summary-panel",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/whatsapp-summary-panel",
					"More_Option_" + i);
