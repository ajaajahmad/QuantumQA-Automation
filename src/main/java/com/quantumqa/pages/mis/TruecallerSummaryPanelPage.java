...
public class TruecallerSummaryPanelPage extends BasePage {
	...
	public TruecallerSummaryPanelPage(WebDriver driver) {
		super(driver);
	...
	public void openTruecallerSummary(String dateValue) {
		menu.navigate("Reports", "Truecaller", "Summary");
		sleep();
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/truecaller-summary-panel",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/truecaller-summary-panel",
					"More_Option_" + i);
