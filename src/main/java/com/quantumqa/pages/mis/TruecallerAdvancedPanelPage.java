...
public class TruecallerAdvancedPanelPage extends BasePage {
	...
	public TruecallerAdvancedPanelPage(WebDriver driver) {
		super(driver);
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/truecaller-advanced-panel",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i,
					"reports/truecaller-advanced-panel", "More_Option_" + i);
