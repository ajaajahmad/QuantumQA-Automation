...
public class WhatsAppAdvancedApiPage extends BasePage {
	...
	public WhatsAppAdvancedApiPage(WebDriver driver) {
		super(driver);
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/whatsapp-advanced-api",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/whatsapp-advanced-api",
					"More_Option_" + i);
