...
public class UserAccountMisTruecallerApiPage extends BasePage {
	...
	public UserAccountMisTruecallerApiPage(WebDriver driver) {
		super(driver);
	...
	public void openUserAccountMisTruecallerApi(String startValue, String endValue) {
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/account-mis-truecaller-api",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i,
					"reports/account-mis-truecaller-api", "More_Option_" + i);
