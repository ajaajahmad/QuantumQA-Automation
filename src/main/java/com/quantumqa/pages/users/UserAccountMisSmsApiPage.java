...
public class UserAccountMisSmsApiPage extends BasePage {
	...
	private final String[] viewOptions = { "User ID", "Date", "Sender ID", "Operator" };
	private final By summaryTable = By.className("data-table");

	public UserAccountMisSmsApiPage(WebDriver driver) {
		super(driver);
	...
	public void openUserAccountMisSmsApi(String startValue, String endValue) {
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/account-mis-sms-api",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/account-mis-sms-api",
					"More_Option_" + i);
