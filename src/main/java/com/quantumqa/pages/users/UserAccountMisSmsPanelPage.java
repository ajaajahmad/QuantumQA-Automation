...
public class UserAccountMisSmsPanelPage extends BasePage {
	...
	private final String[] viewOptions = { "User ID", "Date & Campaign", "Date", "Campaign" };
	private final By summaryTable = By.className("data-table");

	public UserAccountMisSmsPanelPage(WebDriver driver) {
		super(driver);
	...
	public void openUserAccountMisSmsPanel(String startValue, String endValue) {
		menu.navigate("Users", "Account MIS", "SMS");
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/account-mis-sms-panel",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/account-mis-sms-panel",
					"More_Option_" + i);
