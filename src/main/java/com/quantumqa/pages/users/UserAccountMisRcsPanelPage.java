...
public class UserAccountMisRcsPanelPage extends BasePage {
	...
	public UserAccountMisRcsPanelPage(WebDriver driver) {
		super(driver);
	...
	public void openUserAccountMisRcsPanel(String startValue, String endValue) {
		sleep();
		menu.navigate("Users", "Account MIS", "RCS");
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/account-mis-rcs-panel",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/account-mis-rcs-panel",
					"More_Option_" + i);
