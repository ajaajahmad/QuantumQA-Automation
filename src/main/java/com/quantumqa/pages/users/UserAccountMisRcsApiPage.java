...
public class UserAccountMisRcsApiPage extends BasePage {
	...
	public UserAccountMisRcsApiPage(WebDriver driver) {
		super(driver);
	...
	public void openUserAccountMisRcsApi(String startValue, String endValue) {
	...
	public void applyAllViewByOptions(String fileTitle) throws IOException {
		...
		for (String option : viewOptions) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - " + option, "reports/account-mis-rcs-api",
		...
		for (int i = 0; i < 11; i++) {
			...
			HtmlReportUtil.createHtmlTable(table, fileTitle + " - More Option " + i, "reports/account-mis-rcs-api",
					"More_Option_" + i);
